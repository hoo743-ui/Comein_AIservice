...
| 프론트 테스트 | ✅ **29 passed** |
| 백엔드 테스트 | ✅ **15 passed** (§16 에서 죽은 경로 테스트 12개를 지운 뒤의 수) |
| `tsc --noEmit` | ✅ 통과 |
...
2. **Gemini 타임아웃이 Groq 으로 넘어가지 않았다.** `FallbackProvider` 는 `LLMError` 만 잡는데
   ...
   **배포본은 아직 이 고침이 없다.** 같은 문장을 배포 백엔드에 다섯 번 넣었더니:
   ...
   "AI 없이 정리했어요" 를 띄운다. → **이 고침을 배포해야 닫힌다.**
...
0. **이번 고침을 배포할 것** — Render 는 `main` 푸시로 자동 배포된다(`render.yaml: autoDeploy`).
   §20.3-2 의 5분의 1 실패는 **배포해야 닫힌다.** Render 대시보드에 `GROQ_API_KEY` 가
   실제로 들어 있는지도 함께 확인한다 — 폴백이 열려도 받을 쪽이 없으면 소용없다.
...
> 거짓말한다" 였다. 이번엔 **"정적인 검사는 정적인 것만 본다"** 다. 8가지 조합을 전부 통과한
> 대비 검사가, 정작 제품의 클라이맥스인 6초짜리 한 줄을 한 번도 보지 않았다.
