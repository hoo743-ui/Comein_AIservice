...
| §23 | 버튼과 통일성 재점검 |
| §24 | 사용 가이드 아홉 걸음 |
| §25 | **저장소에 없는 파일로 빌드가 무너져 있었다** · 갈래를 줄였다 · 폴백이 이미 죽어 있었다 · 낙관적 쓰기가 거짓말하던 자리 — 지금 상태 |
...
> 한 가지 남겨 둔다: gpt-oss-120b 는 "나 그때 다른 일정 있어서 안 될 것 같아" 에도
> `ask: "언제 일정이 있나요?"` 를 붙였다. 거절인데 되묻는다. qwen 은 올바르게 아무것도
> 내지 않았다. 프롬프트에 거절 규칙 한 줄이 필요할 수 있다 — 폴백에서만 보이는 자리라
> 급하지 않지만, 적어 둔다.
