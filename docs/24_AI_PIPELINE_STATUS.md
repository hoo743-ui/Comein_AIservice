...
| §23 | 버튼과 통일성 재점검 |
| §24 | 사용 가이드 아홉 걸음 |
| §25 | **저장소에 없는 파일로 빌드가 무너져 있었다** · 그리고 갈래를 줄였다 — 지금 상태 |
...
`tsc` 0건 · 프론트 시험 76개 · `next build` 통과(환경변수 없이도) · `pytest` 37개.

시험이 78 → 76 인 것은 없어진 슬라이스를 재던 둘을 함께 걷었기 때문이다 —
없는 것을 재는 시험은 통과해도 아무것도 지키지 않는다.
