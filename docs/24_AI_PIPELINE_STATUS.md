...
3. 미착수 — `/api/chat` 역질문 + `context` 채우기. (모바일 웹 점검은 아래 13절로 옮겼다.)
...
1. **레일 버튼이 39×40px** — 40 은 흔한 하한이지만 44 에는 못 미친다. `--nav-row` 를 손가락 기기에서만
   키우면 되지만, 그 값은 레일의 리듬과 활성 인디케이터가 함께 쓴다. **실제 기기로 보고 정한다.**
2. 실기기 확인 미실시 — 여기까지는 전부 데스크톱 브라우저의 좁은 폭이다. `(pointer: coarse)` 갈래는
   미디어 쿼리를 강제로 켜서 확인했다. **손으로 만져 본 적은 아직 없다.**
